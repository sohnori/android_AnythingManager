...
public class MyProcess6Activity extends Activity {
	GameView mView;
	@Override
	public void onCreate(Bundle savedInstanceState) {
		...
		//setContentView(R.layout.activity_process6);
	}
	...
	class GameView extends View{
		...
		int mMaxNum = 6;	// 숫자의 갯수 최대
		int mWidth = 7;		// 게임판의 가로 크기 최대
		int mHeight = 8;	// 게임판의 세로 크기 최대
		int mTileWidth, mTileHeight;	// 타일 하나의 크기
		int mTileMargin;	//타일 간의 간격
		...
		int mRemoveDelay = 200;	// 타일 제거 후 대기 시간
		...
		int[] arTextColor = {0, Color.RED, Color.GREEN, Color.BLUE, Color.YELLOW, 
				Color.CYAN, Color.MAGENTA, Color.BLACK, Color.GRAY, Color.WHITE};
		// 게임판 배열
		...
		public void onDraw(Canvas canvas) {
			...
			if(mStarting==false) {
				timert.right = Math.max(mTimeRect.right-mEllapse*mTimeRect.width()/mGameTime, 
						mTimeRect.left);										
			}
			...
			for(int x=0;x<mWidth;x++) {
				for(int y=0;y<mHeight;y++) {
					...
					canvas.drawText(""+mBoard[x][y], tx+mTileWidth/2, ty+mTileHeight/2+texty, mTextPaint);					
				}
			}
		}
		
		public boolean onTouchEvent(MotionEvent event) {
			// 에니메이션 중에는 터치 입력을 무시한다.
			if(mStatus !=PLAY) {
				return true;
			}
			...
			public void handleMessage(Message msg) {
				...
				case 3:
					if(checkContinuousTile()) {
						...
						if(mStarting) {
							mStarting = false;
							// 규칙에 맞지 않는 벽돌 완전 제거 후 타이머를 시작한다.
							mHandler.sendEmptyMessage(100);
						}						
					}
					invalidate();
